...
@Entity
@Table(name = "bills")
public class BillEntity extends TaxLinkedEntity {
  @Id
  @UuidGenerator
  ...
  public ValidationResult validateForUpsert(
    ...
    if (dueAt != null && occurredAt != null && dueAt.isBefore(occurredAt)) {
      out.add(new ValidationIssue("dueAt", "dueAt cannot be before occurredAt", Severity.ERROR,
        Map.of("dueAt", dueAt.toString(), "occurredAt", occurredAt.toString())));
    }

    if (dueAt != null) {
      if (DateValidator.isTooOld(dueAt, clock, occurredAtPolicy.maxAge()))
        out.add(new ValidationIssue("dueAt", "dueAt exceeds maxAge", Severity.WARN, Map.of("maxAge", occurredAtPolicy.maxAge().toString())));
    }
