...
public sealed class BillController(BillService service) : ControllerBase
{
    [HttpGet]
    [Authorize(Policy = PermissionPolicies.FinanceRead)]
    public async Task<ActionResult<PagedResult<BillEntity>>> List(
    ...
    public async Task<ActionResult<BillEntity>> Create([FromBody] BillCreateRequest req, CancellationToken ct)
    {
