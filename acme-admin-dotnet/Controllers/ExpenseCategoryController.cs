...
public sealed class ExpenseCategoryController(ExpenseCategoryService service) : ControllerBase
{
    [HttpGet]
    [Authorize(Policy = PermissionPolicies.FinanceRead)]
    public async Task<ActionResult<PagedResult<ExpenseCategoryEntity>>> List(
    ...
    public async Task<ActionResult<ExpenseCategoryEntity>> Create([FromBody] ExpenseCategoryCreateRequest req, CancellationToken ct)
    {
