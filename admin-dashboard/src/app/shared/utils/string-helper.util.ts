...
export default class StringHelper {
  ...
  public static invalidAttrChars = (): RegExp => /[\s"'<>\/=~!@#$%^&*()+=|{}[\];"\\,<>?]/g;
  ...
  public static capitalize(v: string): string {
    if (!v?.length) return '';
    return v.length === 1 ? v.toUpperCase() : `${v.charAt(0).toUpperCase()}${v.slice(1)}`;
  }
  ...
  public static pascalToSnake(v: string): string {
    ...
    if (!this.isUpperCase(v.charAt(0))) return !this.isUpperCase(v) ? v : this.camelToSnake(v);
    return `${v.charAt(0).toUpperCase()}${v
      .slice(1)
      .replace(StringHelper.capitalizedLatin(true), '$1_$2')}`;
  }
  public static camelToSnake(v: string): string {
    if (!v || (typeof v === 'string' && !StringHelper.capitalizedLatin().test(v))) return v;
    v = StringHelper.spaceToUnderscore(v);
  ...
  public static snakeToCamel(v: string): string {
    ...
    return `${v.slice(0, 2)}${v
      .slice(2)
      .replace(new RegExp(`(?:_{2,})([${StringHelper.LOWERCASE_ACCENTED}])`, 'g'), `_$1`)
  ...
  public static kebabToCamel(str: string): string {
    ...
    try {
      return str
        .split('-')
        .map((fragment, i) => (i === 0 ? fragment : StringHelper.capitalize(fragment)))
        .join('');
    } catch (e) {
      return iniStr;
    }
  ...
  public static slugify(v: string, lower: boolean = true): string {
    return lower
      ? this.removeEmojis(this.removeDiacritical(v))
          .replace(new RegExp(`[^${StringHelper.combinedLatinDigits()}]+`, 'g'), '-')
          .replace(/^-+|-+$/g, '')
      : this.removeEmojis(this.removeDiacritical(v))
          .replace(new RegExp(`[^${StringHelper.combinedLatinDigits()}]+`, 'g'), '-')
          .replace(/^-+|-+$/g, '');
  ...
  public static isSaneAttrName(v: string): boolean {
    return /^[a-zA-Z_:]/.test(v) && !/^[0-9]/.test(v) && !StringHelper.invalidAttrChars().test(v);
  }
  ...
  public static scapeToConstructRegExp(v: string, flags: string | undefined): RegExp {
    return new RegExp(v.replace(StringHelper.escapableRegexChars(), '\\$&'), flags);
  }
  ...
  public static padToISO(v: number | string, to: number = 2, using: PseudoNum = '0'): string {
